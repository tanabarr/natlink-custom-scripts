...
class ThisGrammar(GrammarBase):
    ...
    msgPy = 'Messages from NatLink'
    def gotResults_switchToNthWindow(self, words, fullResults):
        #toMimic = ' '
        #[toMimic.join(word[0]) for word in fullResults[3r]]
        wordsToMimic = [word[0] for word in fullResults[3:]]
        natlink.recognitionMimic(['switch', 'to'] + wordsToMimic)
        sleep(0.5)
        natlink.recognitionMimic(['switch', 'to'] + wordsToMimic)
        #natlink.recognitionMimic(' '.join(fullResults[3:][0]))

    def gotResults_showNatLinkWindow(self, words, fullResults):
...
#    @sanitise_movement
        ...
        sleep(0.5)
        natlink.playEvents([(event, x, y)])
        sleep(0.5)
        natlink.playEvents([(event + 1, x, y)])
