...
class ThisGrammar(GrammarBase):
    ...
    gramSpec = """
        ...
        <231> = 'packet to dump' ;
        <309> = 'packet dump' ;
        <232> = 'remote login' ('root' ) ('cluster' | 'local cluster' ) ;
        <311> = 'remote login' ('root' ) ;
        <310> = 'remote login' ('cluster' | 'local cluster' ) ;
        <312> = 'remote login' ;
        <233> = 'multiple get' ;
        ...
        <237> = 'skip on' ;
        <238> = ('pool' | 'she said' ) ('history' | 'status' | 'list' | 'destroy' | 'create' ) ('go' ) ;
        <314> = ('pool' | 'she said' ) ('history' | 'status' | 'list' | 'destroy' | 'create' ) ;
        <239> = 'pool' ('import' | 'export' ) ('go' ) ;
        <316> = 'pool' ('import' | 'export' ) ;
        <315> = 'pool' ('go' ) ;
        <317> = 'pool' ;
    ...
    def gotResults_162(self, words, fullResults):
        ...
        try:
            ...
            handle_error('screen.vch', 45, '\'screen set up chrome\'', e)
            self.firstWord = -1
