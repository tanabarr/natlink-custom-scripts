__version__ = "$Revision: 426 $, $Date: 2011-04-21 16:47:05 +0200 (do, 21 apr 2011) $, $Author: quintijn $"
...
logFolder = os.path.join(natqh.getUnimacroUserDirectory(), language + "_log", natqh.getUser())
...
<inoops2> exported = Cancel | OK | (Choose Format (1 | 2 | 3| 4));
    ...
    def gotBegin(self,moduleInfo):
        ...
        if self.logging and self.prevModinfo != moduleInfo:
            lTime = time.localtime(time.time())
            #print 'logging gotBegin: %s'% lTime
            if logHour != lTime[3]:
                #print 'get new logfilename'
                getLogFileName()
            prog = os.path.split(moduleInfo[0])[1]
            minutes = time.strftime("%H:%M", time.localtime(time.time()))
