...
<replace> exported = replace {placeholders} [with <dgndictation>]; 
    ...
    def gotResults_command(self, words, fullResults):
        ...
        if self.hasCommon(words, ['label']):
            ...
            if label:
                ...
                keystroke('{enter}\\label{{}%s}{enter}'%(self.makes_label(label, contents)))
    ...
    def gotResults(self, words, fullResults):
        if self.floating:
            keystroke('\\begin')
            keystroke('{')
            keystroke(self.floating)
            keystroke('}')
            keystroke('{enter}')
            if self.dictation:
                keystroke ('\\label{')
                keystroke((self.makes_label(self.floating, self.dictation)))
                keystroke('}')
            keystroke ('{enter}\\end{')
            keystroke((self.floating))
            keystroke('}{up}')
        ...
        if self.reference:
            keystroke ('\\ref{%s}' % (self.makes_label(self.reference, self.dictation)))
        if self.namereference:
            keystroke ('\\nameref{%s}' % (self.makes_label(self.namereference, self.dictation)))
        if self.label_text:
            keystroke ('\\label{%s}' % (self.makes_label(self.label_text, self.dictation)))
        if self.replace_text:
            keystroke(self.dictation)
