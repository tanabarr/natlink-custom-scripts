...
class ThisGrammar(ancestor):
    ...
    iniIgnoreGrammarLists = ['count', 'effcount', 'character']
    name = 'keystrokes'
...
##                  (self.GetName(), mode, self.repkeySections, self.norepkeySections)
    ...
    def gotResults_click(self, words, fullResults):
        ...
        if self.key:
            self.flushAll()
        if self.prevRule is None or self.prevRule == 'startrule':
    ...
    def doWaitForMouseToStop(self):
        ...
        """
        if action("MOUSEISMOVING"):
            if action("WAITMOUSESTOP"):
                return 1
            else:
