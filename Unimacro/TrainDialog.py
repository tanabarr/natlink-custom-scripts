    ...
    def gotResultsTraining(self,recogType,resObj):
        if recogType == 'reject':
            ...
            if (rule<=2):
                ...
                results=string.join(resObj.getWords(0))
                print results

                #go next or repeat
                lowResults=string.lower(results)
                lowSpokenForm=string.split(lowResults,'\\')[-1]
                lowPhrase=string.lower(self.dlg.phrase)
...
#    def __init__(self, title, hierList, bitmapID = win32ui.IDB_HIERFOLDERS, dlgID = win32ui.IDD_TREE, dll = None, childListBoxID = win32ui.IDC_LIST1):
    ...
    def SetPhraseToTrain(self):
        ...
        if self.AreTraining:
            ...
            self.SetDlgItemText(IDC_PHRASE,string.center(self.phrase,90))
        ...
        if len(self.trainedPhrases)>0:
            ...
            for (resObj,phrase) in self.trainedPhrases:
                ...
                if not resObj.correction(phrase):
                    if resObj.correction(string.lower(phrase)):
