...
unimacroDirectory = natqh.getOriginalUnimacroDirectory().lower()
if unimacroDirectory not in sampleBases:
    sampleBases.append(unimacroDirectory)
...
#if userDirectory == baseDirectory:
#    print '\n----\nThe UnimacroUserDirectory (%s) (where your ini files are)\n' \
#          'is the same as the Natlink User Directory (UserDirectory), \nwhere the grammar files (of Unimacro) are.\n' \
#          'Please consider choosing a separate UnimacroUserDirectory\n' \
#          'for example a subdirectory of your Documents directory \n(like "[My ]Documents\\Natlink\\Unimacro").\n----\n'% userDirectory
#    
...
if not os.path.isfile(inifile):
    print '---try to find actions.ini file in old version (UserDirectory) or sample_ini directory'
    if userDirectory != baseDirectory and os.path.isfile(oldversioninifile):
        print '----copy actions.ini\nfrom %s\nto new location %s\n---'% (oldversioninifile, inifile)
        shutil.copyfile(oldversioninifile, inifile)
        print '----remove old actions.ini from %s\n---'% (oldversioninifile)
        ...
        for sample in sampleDirectories:
            sampleinifile = os.path.join(sample, 'actions.ini')
            if os.path.isfile(sampleinifile):
                print '---copy actions.ini from\nsamples directory: %s\nto %s\n----'% (sampleinifile, inifile)
                shutil.copyfile(sampleinifile, inifile)
                break
        else:
            print 'no valid sample "actions.ini" file found in one of %s sample directories:\n|%s|'\
                      (len(sampleDirectories), sampleDirectories)
...
Copies of this file could not be found in either:
- userDirectory of Natlink (where this "actions" module is
       (%s) or
- samples of one of the following: (%s).

Also see http://qh.antenna.nl/unimacro/installation/inifilestrategy.html
---------


"""% (userDirectory, baseDirectory, sampleDirectories)
    time.sleep(0.2)
    raise ActionError('no inifile found for actions in unimacro or sample_ini')


if userDirectory != baseDirectory and os.path.isfile(oldversioninifile):
    print 'remove actions.ini from baseDirectory (obsolete): %s'% oldversioninifile
    os.remove(oldversioninifile)
...
def doAction(action, completeAction=None, pauseBA=None, pauseBK=None,
    ...
    if not completeAction:
        ...
        if pauseBA == None:
            pauseBA = int(setting('pause between actions', '100', sectionList=sectionList))
        if pauseBK == None:
            pauseBK = int(setting('pause between keystrokes', '0', sectionList=sectionList))
        ...
        for a in aList:
            ...
            if debug > 4:D('pause between actions: %s'% pauseBA)
            do_W(pauseBA)
        return result
    else:
...
def setPosition(name, pos):
    ...
    iniFileDate = natqh.getFileDate(inifile)
    checkForChanges = 1
    ini.set('positions', name, pos)
    natqh.Wait(0.1)
    ini.write()

def getPosition(name):
    """get from inifile, section 'positions'"""
    return ini.getInt('positions', name) or 0
# -----------------------------------------------------------
...
if debug:
    try:
        ...
        print '_actions, writing debug statements to: %s'% debugFile
    except IOError:
