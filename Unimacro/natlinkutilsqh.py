"$Revision: 550 $, $Date: 2014-10-24 16:47:53 +0200 (vr, 24 okt 2014) $, $Author: quintijn $"
...
def getLanguage():
    ...
    if not lang:
        ...
        if currentUser:
            ...
            lang = natlinkmain.language
            if not lang:
                print 'warning: no language found with current user %s, set language to "zyx"'% currentUser
                return 'zyx'
        else:
...
def getOriginalUnimacroDirectory(fromGetUserDirectory=None):
    ...
    """
    if not fromGetUserDirectory:
        oud = getUserDirectory()
        if oud.lower().endswith('unimacro'):
            return oud
    return natlinkstatus.getOriginalUnimacroDirectory()
...
def getUnimacroUserDirectory():
    ...
    """
    uud = natlinkstatus.getUnimacroUserDirectory()
    if uud:
        #print 'UnimacroUserDirectory: %s'% uud
        if os.path.isdir(uud):
            return uud
    uud = getUserDirectory()
    #print 'take %s for UnimacroUserDirectory'% uud
    return uud
...
def doMouse(absorrel, screenorwindow, xpos, ypos, mouse='left', nClick=1, modifier = 0):
    ...
    if screenorwindow not in [0,1,2,5]:  # only within current window
        print "doMouse, only screenorwindow 0 or 1 or 2 or 5 valid:", screenorwindow
        return
    ...
    elif screenorwindow in (0, 1, 5): 
        if absorrel == 1: # relative:
            xp, yp = relToCoord(xpos, xMin, xMax), relToCoord(ypos, yMin, yMax)
        else:
            xp, yp = getMouseAbsolute(xpos, xMin, xMax), getMouseAbsolute(ypos, yMin, yMax)
...
def buttonClick(button=1, nclick=1):
...
def getMouseAbsolute(Pos, Min, Max):
    if Pos >= 0:
        return min(Min+Pos, Max-1)
    else:
        return max(Max+Pos,Min)


cornerDict = {0:"top left", 1:"top right", 2:"bottom left", 3:"bottom right"}
whichDict = {0: "whole screen", 1:"active window", 5:"client area"}
...
waitingCanceled = 0
iconState = 0
iconDirectory = os.path.join(getUserDirectory(), 'icons')
...
try:
    ...
    print 'warning: cannot open logFolder'
